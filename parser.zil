...
;<GLOBAL GWIM-DISABLE <>> ;"not referenced, removed 2/27/85 by SEM"
...
<ROUTINE PARSER ("AUX" (PTR ,P-LEXSTART) WRD (VAL 0) (VERB <>)
		    (OF-FLAG <>) LEN (DIR <>) (NW 0) (LW 0) NUM SCNT (CNT -1)) 
	<REPEAT ()
	...
	<SETG L-WINNER ,WINNER> ;"for AGAIN"
	...
	<COND (<0? ,P-LEN>
	       ...
	       <COND (<AND ,TUTORIAL-MODE <NOT ,BEG-FLAG>>
		      <SETG BEG-FLAG T>
		      <TELL "|
(That message meant that you didn't type anything at the prompt.)" CR>)>
	       <RFALSE>)>
	<SET LEN ,P-LEN>
	<SETG P-DIR <>>
	<SETG P-NCN 0>
		       ...
		       <COND (<OR <EQUAL? .WRD ,W?THEN ,W?PERIOD>
			     ...
			     (<OR <SET VAL <WT? .WRD ,PS?PREPOSITION 0>>
				  <AND <OR <EQUAL? .WRD ,W?ALL ,W?ONE ;,W?A>
					   ...
					   <WT? .WRD ,PS?OBJECT>>
				       <SET VAL 0>>>
			      <COND (<AND <G? ,P-LEN 1> ;"was >0 until 3/11/85"
					  <==? <GET ,P-LEXV
						    <+ .PTR ,P-LEXELEN>>
					       ,W?OF>
					  <0? .VAL>
...
"There were too many nouns in that sentence." CR>
			     ...
			     (<AND <EQUAL? .VERB ,ACT?TELL>
			      ...
			      <TELL
"To give commands to another character, type the character's name followed
by a comma followed by the command itself. For example:|
|
FLOYD, GIVE ME THE BEDISTOR|" CR>
			      <RFALSE>)
			     (T
			      <CANT-USE .PTR>
			      <RFALSE>)>)
	...
	<SETG P-WALK-DIR <>>
	<COND (<NOT <ZERO? ,P-OFLAG>> <ORPHAN-MERGE>)>
...
<GLOBAL P-NUMBER 0>
;<GLOBAL P-DIRECTION 0> ;"not referenced, removed 2/27/85 by SEM"
...
;"Newer ORPHAN-MERGE, ACLAUSE-WIN, and NCLAUSE-WIN 3/5/85"
<ROUTINE ORPHAN-MERGE ("AUX" (CNT -1) TEMP VERB BEG END (ADJ <>) WRD) 
   <SETG P-OFLAG <>>
...
<ROUTINE UNKNOWN-WORD (PTR "AUX" BUF) 
	...
	<TELL "\"." CR>
	<COND (<AND <NOT ,UNKNOWN-FLAG> ,TUTORIAL-MODE>
	       <SETG UNKNOWN-FLAG T>
	       <TELL CR
"(Here is an example in which a word you typed wasn't in the vocabulary
of the story. Infocom's interactive fiction recognizes more than 600 words
(some over 1000) in your inputs, far more than most games, but there are
limits. In fact, many words used in the story's description won't be
recognized in your inputs. These words are included merely to enhance your
mental imagery of locations and events. If a word you use isn't understood,
it is not important to reaching your goal in the story.)" CR>)> 
	<SETG QUOTE-FLAG <>>
	<SETG P-OFLAG <>>>


<GLOBAL UNKNOWN-FLAG <>>
<GLOBAL CANT-USE-FLAG <>>
...
<ROUTINE CANT-USE (PTR "AUX" BUF) 
	...
	<WORD-PRINT <GETB <REST ,P-LEXV <SET BUF <* .PTR 2>>> 2>
		    <GETB <REST ,P-LEXV .BUF> 3>>
	<TELL "\" here." CR>
	<COND (<AND <NOT ,CANT-USE-FLAG> ,TUTORIAL-MODE>
	       <SETG CANT-USE-FLAG T>
	       <TELL CR
"(This message means that the game knows the word you were using but
couldn't understand the way you used it in the sentence you typed.
Perhaps you should try something with a similar meaning.)" CR>)>
	<SETG QUOTE-FLAG <>>
	<SETG P-OFLAG <>>>
...
<ROUTINE SYNTAX-CHECK ("AUX" SYN LEN NUM OBJ
	...
	<COND (<0? <SET VERB <GET ,P-ITBL ,P-VERB>>>
	       ...
	       <COND (<NOT <ZERO? ,TUTORIAL-MODE>>
		      <TELL "|
(You must begin each sentence with a verb or a special command, like
\"INVENTORY\", for it to be understood.)" CR>)>
	       <RFALSE>)>
		...
		<COND (<DLESS? LEN 1>
		       <COND (<OR .DRIVE1 .DRIVE2> <RETURN>)
			     (T
			      ...
			      <COND (<AND ,TUTORIAL-MODE <NOT ,BAD-SYNTAX>>
				     <SETG BAD-SYNTAX T>
				     <TELL "|
(This warning message means that the type of sentence you entered was
not one that the game normally understands. You should first try to
rephrase it somewhat differently. If that doesn't work, the idea that
you are trying to get across is unknown and therefore unimportant to
the successful completion of the game.)" CR>)>
			      <RFALSE>)>)
	...
	<COND (<AND .DRIVE1
	      ...
	      (<NOT <==? ,WINNER ,PLAYER>>
	       <CANT-ORPHAN>)	      
	      (<G? <GET ,P-OCLAUSE ,P-MATCHLEN> 23>
	       <PUT ,P-OCLAUSE ,P-MATCHLEN 0>
	       <TELL
"You'd better try typing your sentence from the beginning." CR>
	       <RFALSE>)
	      (T
	       ...
	       <TELL "?" CR>
	       <COND (<AND <NOT ,WHAT-TOLD> ,TUTORIAL-MODE>
		      <SETG WHAT-TOLD T>
		      <TELL CR
"(Here's an example of a case in which the game felt that a sentence
was incomplete and couldn't, or wouldn't, figure out how to complete
it. Simply answer the question asked or retype the entire sentence.
Remember that in Infocom's interactive fiction, you can type complete
sentences rather than two-word commands. \"PLACE THE LAMP AND THE SWORD
IN THE TROPHY CASE\" is a perfectly reasonable kind of thing to type.)" CR>)>
	       <RFALSE>)>> 
...
<ROUTINE GWIM (GBIT LBIT PREP "AUX" OBJ) 
	...
	<COND (<GET-OBJECT ,P-MERGE <>>
	       ...
	       <COND (<==? <GET ,P-MERGE ,P-MATCHLEN> 1>
		      ...
		      <COND (<NOT <==? .OBJ ,HANDS>>
			     <TELL D .OBJ ")" CR>)>
		      <COND (<AND <NOT ,GWIM-TOLD> ,TUTORIAL-MODE>
			     <SETG GWIM-TOLD T>
			     <TELL CR
"(This last statement in parentheses was part of the story, not the
tutorial. The story recognized that something was missing from the
sentence you typed and attempted to figure out what it might have
been. In this case, it figured that the " D .OBJ " was the
only reasonable thing to use.|
|
Unlike many other \"adventure games\", you may type complex sentences when
playing Infocom's interactive fiction. Such sentences as \"PUT THE LAMP
AND THE SWORD IN THE TROPHY CASE\" or \"UNLOCK THE WOODEN DOOR USING THE
RUSTY OLD KEY\" are perfectly reasonable. Let's continue.)" CR CR>)>
		      .OBJ)>)
...
;"Next GLOBAL 6/21/84 for WHICH retrofix"
<GLOBAL P-AND <>>
...
"There seems to be a noun missing in that sentence!" CR>
		       <COND (<AND ,TUTORIAL-MODE <NOT ,NOUN-MISS>>
			      <SETG NOUN-MISS T>
			      <TELL "|
(This message means that the game thought that your sentence should have
another noun in it somewhere. The most common cause of this is an incomplete
sentence such as \"PUT THE LAMP IN\", where the game was expecting another
noun. At other times, the game has misunderstood one of the other words
in your sentence. Try another way of expressing your idea.)" CR>)>)>
		<RFALSE>)>
			    ...
			    (T
			     ...
			     <COND (<NOT <==? ,WINNER ,PLAYER>>
				   ...
				   (<G? <GET ,P-OCLAUSE ,P-MATCHLEN> 23>
				    <PUT ,P-OCLAUSE ,P-MATCHLEN 0>
				    <TELL
"You'd better try typing your sentence again from the beginning." CR>)
				   (<AND .VRB ,P-NAM>
		      ...
		      <COND (.VRB
			     ...
			     <COND (<NOT <ZERO? ,LIT>>
				   ...
				   (T
				    ...
				    <COND (<NOT <ZERO? ,TUTORIAL-MODE>>
					   <TELL "|
(It's dark here in the broom closet. The game won't let you refer
to objects that you cannot see!)" CR>)>)>)>
		      <SETG P-NAM <>>
		      <SETG P-ADJ <>>
		      <RFALSE>)
		     (<0? .LEN> <SET GCHECK T> <AGAIN>)>
	       <SETG P-SLOCBITS .XBITS>
	       <SETG P-XNAM ,P-NAM> ;"for elevator hacking"
	       <SETG P-NAM <>>
	       <SETG P-ADJ <>>
	       <RTRUE>>>
...
<ROUTINE WHICH-PRINT (TLEN LEN TBL "AUX" OBJ RLEN)
	 <SET RLEN .LEN>
	 <TELL "Which">
         <COND (;"P-AND added to OR on 6/21/84 for WHICH retrofix"
	        <OR ,P-OFLAG
		    ,P-MERGED
		    ,P-AND>
		<TELL " ">
	 ...
	 <REPEAT ()
		 ...
		 <COND (<L? <SET LEN <- .LEN 1>> 1>
		        <TELL "?" CR>
		        <RETURN>)>>
	 <COND (<AND <NOT ,WHICH-TOLD> ,TUTORIAL-MODE>
		<SETG WHICH-TOLD T>
		<TELL CR
"(This demonstrates another feature of our interactive fiction: the ability
to use adjectives in sentences. In this case, the word \"">
		<PRINTB ,P-NAM>
		<TELL "\" was confusing because there were ">
		<TELL <GET ,NUM-TBL .RLEN>>
		<TELL " of them in your location. You can answer the
question posed either by typing the entire sentence or, more easily,
by typing an adjective that will clear up the confusion. Do one of
those things now.)" CR>)>>

<GLOBAL NUM-TBL <TABLE 0 "one" "two" "three">>
...
;<ROUTINE PRSO-PRINT ("AUX" PTR) ;"not referenced, removed 2/27/85 by SEM"
	 <COND (<OR ,P-MERGED
		    <==? <GET <SET PTR <GET ,P-ITBL ,P-NC1>> 0> ,W?IT>>
		<TELL " " D ,PRSO>)
	       (T <BUFFER-PRINT .PTR <GET ,P-ITBL ,P-NC1L> <>>)>>

;<ROUTINE PRSI-PRINT ("AUX" PTR) ;"not referenced, removed 2/27/85 by SEM"
	 <COND (<OR ,P-MERGED
...
<ROUTINE GENERIC-COMPLAIN ()
	 <COND (<NOT <ZERO? ,TUTORIAL-MODE>>
		<TELL CR
"(Here is an example of something the game might print when it doesn't
understand what you are saying. When this happens, try something slightly
different. If nothing you are doing seems to work, try a different idea
altogether.)" CR>)>>